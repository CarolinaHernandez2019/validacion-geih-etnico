...
def encontrar_ultimo_periodo(df, fila_periodos):
    """Encuentra el último período (cualquiera) y los últimos N períodos"""
    periodos = {}
    for col in range(1, df.shape[1]):  # Empezar desde col 1 (col 0 es concepto)
        val = df.iloc[fila_periodos, col]
        if pd.notna(val) and str(val).strip():
            periodos[col] = str(val).strip()
    return periodos
...
def filtrar_hoja(df, fila_periodos, num_periodos=4):
    ...
    - Columna A (conceptos)
    - Últimas N columnas de períodos
    """
    periodos = encontrar_ultimo_periodo(df, fila_periodos)
    
    if not periodos:
        return None, []
    
    # Tomar las últimas N columnas
    cols_ordenadas = sorted(periodos.keys())[-num_periodos:]
    nombres_periodos = [periodos[c] for c in cols_ordenadas]
    ...
    return df_filtrado, nombres_periodos
    ...
    try:
        ...
        for hoja_nombre, config in HOJAS_TOTAL_NACIONAL.items():
            if hoja_nombre in xlsx.sheet_names:
                ...
                periodos = encontrar_ultimo_periodo(df, config['fila_periodos'])
                
                if periodos:
                    ultimo_periodo = list(periodos.values())[-1]
                    hojas_encontradas[config['nombre_corto']] = (df, config['fila_periodos'])
                    st.write(f"  ✅ **{hoja_nombre}** → {len(periodos)} períodos, último: **{ultimo_periodo}**")
                else:
